...
**A lane can end without a verdict.** `incomplete`, `timeout`, and `unavailable` all mean the work never reached a conclusion — none of them is evidence that the spec is wrong or that the lane misread it, so none counts toward the two-failures takeover. They differ in what survives. `incomplete` leaves codex running and hands you a resumable `PID=… FINAL=… LOG=…`: tell that same agent to resume. `timeout` means the lane killed codex when the budget ran out, so the partial diff is all there is — re-run the same spec with a larger `BUDGET:` line, split it into pieces that each fit the clock, or keep it yourself, and say in your report which you chose. Do not re-route a `timeout` to `astra-implementer` as a misclassification: that lane runs longer, not shorter. `refused` — codex exited clean with an empty diff — counts as a failure only when it declined the task itself; when its verbatim `REASON` shows it declined over machine-wide config (the `~/.codex/AGENTS.md` case the lanes' preamble exists to pre-empt), fix the cause and re-invoke rather than spending an attempt. Whichever you pick, name the status and the cost change in your report — never quietly absorb either.
