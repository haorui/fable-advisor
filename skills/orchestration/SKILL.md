---
name: orchestration
description: 'Routing doctrine for the architect-as-orchestrator pattern — how a session on any model delegates routine implementation to `luna-implementer`, escalates judgment-heavy one-offs to `sol-implementer`, optionally races the standing lane against `opus-implementer`, consults `fable-advisor` at commitment boundaries, and gets every deliverable reviewed by `fable-advisor` before reporting done. USE WHEN delegating implementation work, choosing between luna-implementer/sol-implementer lanes, routing between the standing and optional race lanes, turning architect mode off ("solo mode", "不用车道", "关闭 architect 模式", or the `fable-advisor lane profile: off` line in CLAUDE.md), turning it back on ("architect mode on", "use lanes", "开启 architect 模式"), choosing a reasoning effort for sol-implementer, writing a spec for a subagent, deciding whether to consult or invoke fable-advisor, using the Codex plugin''s review skills, managing session cost or token spend, or running any multi-task build where the session is the architect.'
---

# Orchestration — the architect's routing doctrine

The session is the architect, running on whatever model the user chose. It owns requirements, architecture, decomposition, specs, routing, and verification. It should almost never type implementation code. Every implementation task gets delegated to the implementation lane the architect routes it to — `luna-implementer` by default, `sol-implementer` for judgment-heavy one-offs — and every finished deliverable gets a review from `fable-advisor` before the architect reports done.
...
| `fable-advisor` | Fable 5.1 (Claude model `fable`, high effort) | Reviewer + outside voice | Two modes: REVIEW (`ship / fix-first / rethink`) and CONSULT (`proceed / revise / rethink`). No external dependency. |
...
How much does the outcome depend on judgment the spec can't capture? Little → `luna-implementer`; a lot, and mistakes are costly → `sol-implementer`, or keep that piece with the architect.

The architect does not route mechanical edits, renames, wiring, boilerplate, config, or pattern-mirroring tests to `sol-implementer` — that is `luna-implementer` work. When the user explicitly sends such work to the lane anyway (vendor diversity, trying the model), it runs at the `medium` baseline and that is a legitimate path, not a routing defect.

If a lane returns `blocked`, surface the denial to the user and never re-send the lane with a reworded prompt. `blocked` does not count toward the two-failures takeover.
...
2. **Consult the outside voice**: send that decision memo to `fable-advisor` in CONSULT mode. It returns proceed / revise / rethink with fresh eyes, judged against the memo rather than the conversation. The non-Anthropic perspective still sits at the implementation lane — GPT writes, Claude/Fable judges.
...
| Sol rung | Move up when |
|---|---|
| `medium` | Baseline. Every Sol spec starts here, including work the user explicitly routed to the lane. |
...
For high-stakes work, race the two implementation lanes on the same spec and let the architect pick the stronger diff — two model families, one judged result. The race is the lane the task was routed to (`luna-implementer`, or `sol-implementer` for an escalation) vs `opus-implementer`; both diffs go to `fable-advisor` as one deliverable, not two.

## The final review — mandatory

**Always, once, at the end of a deliverable:** invoke `fable-advisor`, the review gate, with the stated goal, the constraints, and where to find the changes. It reads the accumulated diff with fresh eyes, judged against the goal rather than the conversation, and returns ship / fix-first / rethink. The architect does not report done before this review. **The architect never substitutes its own self-review for the gate** — the gate is an agent invocation with a returned verdict, or it did not happen.
...
`fable-advisor` has no external dependency, so unavailability is not an expected failure mode; the only non-verdict outcome is `STATUS: insufficient-brief`, and the architect supplies what's missing and re-invokes.
...
What the review buys depends on the session model. When the architect is not Fable 5.1, the consult and review are a second model, typically a stronger one reading in a clean context. When the architect is also Fable 5.1, the review is a fresh-eyes check, not an independent-model check. Act on the verdict or surface the disagreement — never silently ignore it. `fix-first` findings go back through the lane that produced the change as corrected specs.
...
**A lane can end without a verdict.** `incomplete`, `timeout`, `unavailable`, and `blocked` all mean the work never reached a conclusion — none of them is evidence that the spec is wrong or that the lane misread it, so none counts toward the two-failures takeover. They differ in what survives. `incomplete` leaves codex running and hands you a resumable `PID=… FINAL=… LOG=…`: tell that same agent to resume. `timeout` means the lane killed codex when the budget ran out, so the partial diff is all there is — re-run the same spec with a larger `BUDGET:` line, split it into pieces that each fit the clock, or keep it yourself, and say in your report which you chose. Do not re-route a `timeout` to `sol-implementer` as a misclassification: that lane runs longer, not shorter. `refused` — codex exited clean with an empty diff — counts as a failure only when it declined the task itself; when its verbatim `REASON` shows it declined over machine-wide config (the `~/.codex/AGENTS.md` case the lanes' preamble exists to pre-empt), fix the cause and re-invoke rather than spending an attempt. Whichever you pick, name the status and the cost change in your report — never quietly absorb either.

**Packaging is the architect's.** No lane commits — a lane's report describes a verified working tree, not a commit. Once you have read the diff and re-run the verification, you write the commit: the party that accepts the work records it, and commit granularity and message conventions were yours to set in the spec anyway. Never write "commit it yourself" into a spec — beyond blurring that ownership, the codex lanes are sandboxed to the working tree while a git worktree's `.git` metadata lives outside it under the main checkout, so they structurally cannot, and asking spends a round trip to be told so.
