...
| `sol-implementer` | **GPT-5.6 Sol** (max reasoning) | Implementation — high-complexity one-offs where judgment the spec can't capture decides the outcome | Escalation lane, never the default |
...
The Claude agents (`opus-implementer` and `opus-reviewer`) set `effort: high` in their frontmatter; neither codex agent has a frontmatter effort field — `codex-implementer` pins GPT-5.6 Luna to max and `sol-implementer` pins GPT-5.6 Sol to max, both in their Codex CLI invocations.
