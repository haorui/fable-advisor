...
The architect writes the spec, delegates to `codex-implementer` (or races it against `opus-implementer` on a high-stakes spec), reads the diff and verification evidence when the report comes back, sends the finished work through `opus-reviewer` for the final review, and only then reports done.
...
**Upstream versions?** This fork's lineage: upstream v4 (Opus architect, Codex routine lane, Fable escalation + review) → v5 (Fable architect, Codex implementation lane, native Opus review, keeping v4's producing pair with the architect seat upgraded) → this v6 (one fixed arrangement — codex implements, opus reviews; profiles removed). For the original pattern, use [DannyMac180/fable-advisor](https://github.com/DannyMac180/fable-advisor).
